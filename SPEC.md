...
- A node can be re-parented by dragging it onto another node: the dragged node and its whole subtree move under the target, and the branch flips to the target's side when needed (the root accepts both sides, choosing by drop position). The hovered target is highlighted during the drag. Dropping a single tree node on empty canvas reorders it among its siblings by vertical drop position and snaps the branch back into the layout; standalone images and multi-node selections only reposition. A node cannot be dropped onto itself, its current parent, or one of its own descendants. Re-parenting is undoable in one step.
